...
public class SelectionParameter extends StringParameter {
    ...
    public SelectionParameter(String name, boolean isOptional, String defaultValue, String... options) {
        ...
        this.options.add(Arrays.toString(options));
    }

    @Override
    public String parse(Parser parser) throws CommandSyntaxException {
        final int start = parser.getCursor();
        String value = super.parse(parser);
        if (options.contains(value)) {
            return value;
        } else {
            parser.setCursor(start);
            throw BuiltInExceptions.createWithContext("无效的选项，“需要" + getOptionsTable() + "”却出现了" + value, parser);
        }
    }
    ...
    public String toString() {
        if (isOptional) {
            ...
            return CommandDispatcher.USAGE_REQUIRED_OPEN + optionsTable + CommandDispatcher.USAGE_REQUIRED_CLOSE;
        }
    }
}