...
/**
 * Like {@link ConfigSpec} except in builder format, and extended to accept comments, language keys,
...
public class ConfigSpecHelper extends UnmodifiableConfigWrapper<UnmodifiableConfig> implements UnmodifiableConfig {
    ...
    private ConfigSpecHelper(UnmodifiableConfig storage, UnmodifiableConfig values, Map<List<String>, String> levelComments, Map<List<String>, String> levelTranslationKeys) {
        super(storage);
        this.values = values;
    ...
    public static class Builder implements top.spco.core.Builder<ConfigSpecHelper> {
        ...
        public <T> Pair<T, ConfigSpecHelper> configure(Function<Builder, T> consumer) {
            T o = consumer.apply(this);
            return Pair.of(o, this.build());
        }

        @Override
        public ConfigSpecHelper build() {
            context.ensureEmpty();
            ...
            ConfigSpecHelper ret = new ConfigSpecHelper(storage, valueCfg, levelComments, levelTranslationKeys);
    ...
    public static class ConfigValue<T> implements Supplier<T> {
         ...
         * @throws NullPointerException  if the {@link ConfigSpecHelper config spec} object that will contain this has
         *                               not yet been built
        ...
        protected T getRaw(Config config, List<String> path, Supplier<T> defaultSupplier) {
            ...
            // 验证类型是否与泛型 T 一致
            if (!defaultSupplier.get().getClass().isInstance(value)) {
                throw new ClassCastException("Value at " + path + " is not compatible with " + defaultSupplier.get().getClass());
            }
